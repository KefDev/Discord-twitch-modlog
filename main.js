    ...
    if (m.channel.guild != null && !m.author.bot) {
        Client.db.query("SELECT * FROM global WHERE discordID = ?", [m.channel.guild.id], (error, results) => {
            if (error) console.log(error);
                ...
                if (["eval", "reason", "log", "prefix", "lang", "twitch", "help"].includes(command) && m.content.toLowerCase().startsWith(server.prefix)) require(`./commands/${command}.js`).exec(Client, m, server);
